...
class LookupConnectorTest extends AwrsUnitTestTraits {
  ...
  "LookupConnector by urn" should {
    ...
    "lookup an awrs entry when a valid reference number is entered" in {
      ...
      when(mockWSHttp.GET[HttpResponse](Matchers.endsWith(expectedURL))(Matchers.any(), Matchers.any(), Matchers.any())).thenReturn(Future.successful(HttpResponse(OK, lookupSuccess, Map.empty[String, Seq[String]])))
    ...
    "an exception when invalid json is returned" in {
      ...
      val expectedURL = s"""$urnURL$testAwrsRef"""
      implicit val hc = new HeaderCarrier(sessionId = Some(SessionId(s"session-${UUID.randomUUID}")))
      when(mockWSHttp.GET[HttpResponse](Matchers.endsWith(expectedURL))(Matchers.any(), Matchers.any(), Matchers.any())).thenReturn(Future.successful(HttpResponse(OK, invalidJson, Map.empty[String, Seq[String]])))
      val result = TestLookupConnector.queryByUrn(testAwrsRef)
      val thrown = the[InternalServerException] thrownBy await(result)
