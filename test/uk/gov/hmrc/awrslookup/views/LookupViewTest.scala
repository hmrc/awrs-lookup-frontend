...
class LookupViewTest extends AwrsUnitTestTraits with HtmlUtils {
  ...
  "Lookup Controller " should {
    ...
    "display an empty search page landed on for the first time" in {
      val document: Document = TestLookupController.show(false).apply(testRequest(query = None))
      document.getElementById("search-heading").text shouldBe Messages("awrs.lookup.search.heading.revised")
      document.getElementById("search-lede").text should include(Messages("awrs.lookup.search.lede.revised", Messages("awrs.lookup.search.awrs_urn","","","(","),"), Messages("awrs.lookup.search.isle_of_man", "", "")))
      document.getElementById("search-lede").text should include(Messages("awrs.lookup.search.lede.revised", Messages("awrs.lookup.search.awrs_urn","","","(","),")))
